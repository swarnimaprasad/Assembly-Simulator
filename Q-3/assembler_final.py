...
def a_chk_instruction_reg_name(type, input):
    if type == "instruction":
        if input not in ["add", "sub", "mov", "ld", "st", "mul", "div", "rs", "ls", "xor", "or", "and", "not", "cmp", "jmp", "jlt", "jgt", "je", "hlt","addf","subf","movf"]:
...
def j_chkImm(immediate):
    
    if not(0.25 <= float(immediate[1:]) <=15.75) :
        list_error.append("Syntax Error: Illegal immediate value (not between 0.25 and 15.75)")
        return True
    return False
...
def call_list(dict_instruc,i,list_print,list_var,list_labels,dict_instuc_main):
    ...
    elif dict_instruc[i][0]=="addf":
        if len(dict_instruc[i])>4:
            raise
        if a_chk_instruction_reg_name("register",dict_instruc[i][1]) or a_chk_instruction_reg_name("register",dict_instruc[i][2]) or a_chk_instruction_reg_name("register",dict_instruc[i][3]):
            return 1
        list_print.append(addf(dict_instruc[i][1],dict_instruc[i][2],dict_instruc[i][3],dict_opcode,dict_register))
    ...
    elif dict_instruc[i][0]=="subf":
        if len(dict_instruc[i])>4:
            raise 
        if a_chk_instruction_reg_name("register",dict_instruc[i][1]) or a_chk_instruction_reg_name("register",dict_instruc[i][2]) or a_chk_instruction_reg_name("register",dict_instruc[i][3]):
            return 1
        list_print.append(subf(dict_instruc[i][1],dict_instruc[i][2],dict_instruc[i][3],dict_opcode,dict_register))
    ...
    elif dict_instruc[i][0]=="movf":
        if len(dict_instruc[i])>3:
            raise 
        if a_chk_instruction_reg_name("register",dict_instruc[i][1]) or j_chkImm(dict_instruc[i][2]):
            return 1
        list_print.append(movf(dict_instruc[i][1],dict_instruc[i][2],dict_opcode,dict_register))
...
def addf(reg1,reg2,reg3,dict_opcode,dict_register):
    opcode=dict_opcode["addf"]
    reg1=dict_register[reg1]
    reg2=dict_register[reg2]
    reg3=dict_register[reg3]
    binary=opcode+"0"*(16-len(opcode)-len(reg1)-len(reg2)-len(reg3))+reg1+reg2+reg3
...
def subf(reg1, reg2 ,reg3,dict_opcode,dict_register):
    opcode=dict_opcode["subf"]
    reg1=dict_register[reg1]
    reg2=dict_register[reg2]
    reg3=dict_register[reg3]
    binary=opcode+"0"*(16-len(opcode)-len(reg1)-len(reg2)-len(reg3))+reg1+reg2+reg3
...
def movf(reg1,Imm,dict_opcode,dict_register):
    opcode=dict_opcode["movf"]
    reg1=dict_register[reg1]
    Imm=change_imm(Imm)
    Imm=Imm[1:]
    Imm=Imm.split(".")
...
try:
    ...
    with open("assembly.txt","r") as input_file:
        input_start=list(input_file.readlines())
            ...
            if type(dict_instruc[i])==list:
                ...
                for j in dict_instruc[i]:
                    if call_list(dict_instruc[i],j,list_print,list_var,list_labels,dict_instruc)==1:
                        ...
                        error=1
                        break
    with open("bin.txt","w+") as output_file:
        if error==1:
            if type(list_error[1])!=int:
                output_file.write("Error generated at line number: "+str(int("0b"+list_error[1],2)+len(list_var)+1)+"\n"+list_error[0])
            else:
                output_file.write("Error generated at line number: "+str(list_error[1]+len(list_var)+1)+"\n"+list_error[0])
        else:
            if len(list_print)>128:
                output_file.write("Program length exceeded 128 lines.")
            else:
                for i in list_print:
                    output_file.write(i+"\n")
